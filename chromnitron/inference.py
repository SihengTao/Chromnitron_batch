...
def resource_root(input_dict, key):
    # Allow per-resource overrides while keeping a shared root as default
    return input_dict.get(f'{key}_root', input_dict.get('root', ''))

def build_atac_path(input_dict, celltype):
    """Build ATAC path, supporting a configurable pattern with {sample} placeholder."""
    atac_root = resource_root(input_dict, 'atac')
    atac_subdir = input_dict.get('atac', '')
    base_dir = os.path.join(atac_root, atac_subdir)
    pattern = input_dict.get('atac_pattern', '{sample}.zarr')
    try:
        relative_path = pattern.format(sample=celltype)
    except KeyError:
...
def load_data(config, celltype, loci_info, cap, chr_sizes):
    ...
    seq_root = resource_root(input_dict, 'sequence')
    atac_root = resource_root(input_dict, 'atac')
    cap_root = resource_root(input_dict, 'cap')

    input_seq_path = os.path.join(seq_root, input_dict.get('sequence', ''), f'{assembly}.zarr')
    input_features_path = build_atac_path(input_dict, celltype)
    esm_feature_path = os.path.join(cap_root, input_dict.get('cap', ''), f'{cap}.npz')

    if config['inference_config']['input']['excluded_region_path'] == 'auto':
        excluded_region_path = os.path.join(seq_root, input_dict.get('sequence', ''), f'{assembly}-blacklist.v2.bed')
    else:
...
def verify_prediction_exists(config, celltype, cap):
    skip_if_exists = config.get('inference_config', {}).get('output', {}).get('skip_if_exists', True)
    ...
    # Check if any inference output already exists
    output_paths = [
        f'{save_path}/data.npy',
        f'{save_path}/locus.csv',
        f'{save_path}/locus.bed',
    ]
    existing_paths = [path for path in output_paths if os.path.exists(path)]
    if existing_paths:
        existing_files = ', '.join(os.path.basename(path) for path in existing_paths)
        print(f'Inference output already exists for {save_path} ({existing_files}), skipping...')
        return True
    return False
...
def get_chr_sizes(config, chrs):
    ...
    assembly = config["inference_config"]["input"]["assembly"]
    seq_root = resource_root(input_dict, 'sequence')
    chr_path = os.path.join(seq_root, input_dict.get('sequence', ''), f'{assembly}.chrom.sizes')
