...
def skip_if_exists_enabled(config):
    return config.get('inference_config', {}).get('output', {}).get('skip_if_exists', True)
...
def run_peak_calling(config, celltype, cap, data_dict):
    ...
    if skip_if_exists_enabled(config) and os.path.exists(peak_path):
...
def run_store_bigwig(config, celltype, cap, data_dict, chr_sizes):
    ...
    if skip_if_exists_enabled(config) and os.path.exists(bw_path):
...
def run_store_zarr(config, celltype, cap, data_dict, chr_sizes):
    ...
    if skip_if_exists_enabled(config) and os.path.exists(zarr_path):
